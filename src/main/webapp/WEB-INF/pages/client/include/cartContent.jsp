            ...
            <table class="table-cart-checkout mytable table-convert-price">
                ...
                <c:forEach var="sanpham" items="${cart}" varStatus="loop">
                    <tr class="cart_line" id="item${sanpham.id}">
                        <div>
                            <td>${loop.index+1}</td>
                            <td><img src="/ecommerceshop/img/${sanpham.id}.png" style="width: 150px" class="cart-img">
                            </td>
                            <td style="text-align: center; mergin-top: -52px;">
                                <p class="cart_ten"><a
                                        href="<c:url value="/sp?id=${sanpham.id}" />">${sanpham.tenSanPham}</a></p>
                            ...
                            <td>
                                <input class="input_num_cart" type="number" value="${quanity[sanpham.id]}" min="1"
                                       onChange="changeQuanity(${sanpham.id},this.value,${sanpham.donGia})">
                            </td>
                            <td><b><span class="total" id="item${sanpham.id}_total"
                                         name="total_price">${sanpham.donGia*quanity[sanpham.id]}</span> VND</b></td>
                            <td class="cart-img">
                    ...
                    <c:if test="${checkEmpty != 0 }">
                        <td colspan="3">
                            <a class="btn btn-primary" href="<%=request.getContextPath()%>/"><span
                                    class="glyphicon glyphicon-hand-left"></span> Mua thêm sản phẩm khác</a>
                            &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
                            <a class="btn btn-warning " href="<%=request.getContextPath()%>/checkout"><span
                                    class="glyphicon glyphicon-check"></span> Thanh toán</a>

<%--                            <a class="btn btn-danger " id="pay-online-button" ><span--%>
<%--                                    class="glyphicon glyphicon-check"></span> Thanh toán online</a>--%>
                        </td>
                    </c:if>
